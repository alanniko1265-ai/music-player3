/**
 * QQ 音乐请求签名模块
 * 移植自 QQMusicApi/util/sign.js
 *
 * 该模块包含 QQ 音乐 API 请求签名算法。
 * 算法核心是一个自定义 MD5 哈希函数和一个虚拟机解释器，
 * 两者协同工作生成最终的签名字符串。
 */
...
/**
 * 初始化签名环境
 * 在隔离的 VM 上下文中执行原始签名算法代码
 */
function initSignEnvironment(): (data: string) => string {
  ...
  // Make global self-referential
  sandbox.global = sandbox

  vm.createContext(sandbox)

  // Execute the original sign.js algorithm
  const code = getSignAlgorithmCode()
  vm.runInContext(code, sandbox)

  const getSecuritySign = sandbox.__getSecuritySign
  if (typeof getSecuritySign !== 'function') {
    throw new Error('Failed to initialize QQ Music sign module: __getSecuritySign not found')
  }

  return getSecuritySign
}

/**
 * 返回原始签名算法的 JavaScript 代码
 */
function getSignAlgorithmCode(): string {
  return `
var O = (function() {
...
O.__sign_hash_20200305 = function(e) {
  ...
  function t(e) {
    return function(e) {
      ...
      var t, o = [];
      for (o[(e.length >> 2) - 1] = void 0, t = 0; t < o.length; t += 1)
        o[t] = 0;
...
(function s(c, l, u, d, p) {
  p = p || [[this], [{}]];
  for (var t = [], o = null, e = [function() { return !0; }, function() {}, function() { p.length = u[l++]; }, function() { p.push(u[l++]); }, function() { p.pop(); }, function() { var e = u[l++], t = p[p.length - 2 - e]; p[p.length - 2 - e] = p.pop(), p.push(t); }, function() { p.push(p[p.length - 1]); }, function() { p.push([p.pop(), p.pop()].reverse()); }, function() { p.push([d, p.pop()]); }, function() { p.push([p.pop()]); }, function() { var e = p.pop(); p.push(e[0][e[1]]); }, function() { p.push(p[p.pop()[0]][0]); }, function() { var e = p[p.length - 2]; e[0][e[1]] = p[p.length - 1]; }, function() { p[p[p.length - 2][0]][0] = p[p.length - 1]; }, function() { var e = p.pop(), t = p.pop(); p.push([t[0][t[1]], e]); }, function() { var e = p.pop(); p.push([p[p.pop()][0], e]); }, function() { var e = p.pop(); p.push(delete e[0][e[1]]); }, function() { var e = []; for (var t in p.pop()) e.push(t); p.push(e); }, function() { p[p.length - 1].length ? p.push(p[p.length - 1].shift(), !0) : p.push(void 0, !1); }, function() { var e = p[p.length - 2], t = Object.getOwnPropertyDescriptor(e[0], e[1]) || { configurable: !0, enumerable: !0 }; t.get = p[p.length - 1], Object.defineProperty(e[0], e[1], t); }, function() { var e = p[p.length - 2], t = Object.getOwnPropertyDescriptor(e[0], e[1]) || { configurable: !0, enumerable: !0 }; t.set = p[p.length - 1], Object.defineProperty(e[0], e[1], t); }, function() { l = u[l++]; }, function() { var e = u[l++]; p[p.length - 1] && (l = e); }, function() { throw p[p.length - 1]; }, function() { var e = u[l++], t = e ? p.slice(-e) : []; p.length -= e, p.push(p.pop().apply(d, t)); }, function() { var e = u[l++], t = e ? p.slice(-e) : []; p.length -= e; var o = p.pop(); p.push(o[0][o[1]].apply(o[0], t)); }, function() { var e = u[l++], t = e ? p.slice(-e) : []; p.length -= e, t.unshift(null), p.push(new (Function.prototype.bind.apply(p.pop(), t))); }, function() { var e = u[l++], t = e ? p.slice(-e) : []; p.length -= e, t.unshift(null); var o = p.pop(); p.push(new (Function.prototype.bind.apply(o[0][o[1]], t))); }, function() { p.push(!p.pop()); }, function() { p.push(~p.pop()); }, function() { p.push(typeof p.pop()); }, function() { p[p.length - 2] = p[p.length - 2] == p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] === p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] > p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] >= p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] << p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] >> p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] >>> p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] + p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] - p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] * p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] / p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] % p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] | p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] & p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] ^ p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] in p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] instanceof p.pop(); }, function() { p[p[p.length - 1][0]] = void 0 === p[p[p.length - 1][0]] ? [] : p[p[p.length - 1][0]]; }, function() { for (var n = u[l++], i = [], e = u[l++], t = u[l++], r = [], o = 0; o < e; o++) i[u[l++]] = p[u[l++]]; for (var a = 0; a < t; a++) r[a] = u[l++]; p.push(function e() { var t = i.slice(0); t[0] = [this], t[1] = [arguments], t[2] = [e]; for (var o = 0; o < r.length && o < arguments.length; o++) 0 < r[o] && (t[r[o]] = [arguments[o]]); return s(c, n, u, d, t); }); }, function() { t.push([u[l++], p.length, u[l++]]); }, function() { t.pop(); }, function() { return !!o; }, function() { o = null; }, function() { p[p.length - 1] += String.fromCharCode(u[l++]); }, function() { p.push(""); }, function() { p.push(void 0); }, function() { p.push(null); }, function() { p.push(!0); }, function() { p.push(!1); }, function() { p.length -= u[l++]; }, function() { p[p.length - 1] = u[l++]; }, function() { var e = p.pop(), t = p[p.length - 1]; t[0][t[1]] = p[e[0]][0]; }, function() { var e = p.pop(), t = p[p.length - 1]; t[0][t[1]] = e[0][e[1]]; }, function() { var e = p.pop(), t = p[p.length - 1]; p[t[0]][0] = p[e[0]][0]; }, function() { var e = p.pop(), t = p[p.length - 1]; p[t[0]][0] = e[0][e[1]]; }, function() { p[p.length - 2] = p[p.length - 2] < p.pop(); }, function() { p[p.length - 2] = p[p.length - 2] <= p.pop(); }]; ; )
    try {
      for (; !e[u[l++]](); ) ;
      if (o) throw o;
      return p.pop();
      ...
      var n = t.pop();
      if (void 0 === n) throw e;
      o = e, l = n[0], p.length = n[1], n[2] && (p[n[2]][0] = o);
    }
...
// Initialize the sign function once at module load
const getSecuritySign = initSignEnvironment()

/**
 * 对 QQ 音乐 API 请求数据进行签名计算
 *
 * @param data - 请求数据，可以是对象或字符串
 * @returns 签名字符串
 */
export function sign(data: Record<string, unknown> | string): string {
  let str: string
